...
mod post {
    ...
    pub async fn route(
        ...
        permissions.has_server_permission("databases.delete-structure")?;

        if database.database_host.maintenance_enabled {
            return ApiResponse::error(
                "cannot delete column while database host is in maintenance mode",
            )
