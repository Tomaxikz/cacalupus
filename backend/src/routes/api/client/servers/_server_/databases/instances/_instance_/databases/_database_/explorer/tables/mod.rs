...
mod post {
    ...
    use shared::{
        ...
        models::{
            ...
            server_database::{CREATE_TABLE_MAX_COLUMNS, ColumnDefinition},
    ...
    pub struct Payload {
        ...
        #[garde(length(min = 1, max = CREATE_TABLE_MAX_COLUMNS), dive)]
    ...
    pub async fn route(
        ...
        tokio::spawn(async move {
            match client
                .post_instances_instance_databases_database_explorer_tables(
                    database_instance.uuid,
                    database,
                    &db_agent_api::instances_instance_databases_database_explorer_tables::post::RequestBody {
                        schema: data.schema,
            ...
            activity_logger
                .log(
                    "server:database-instance.database.table-create",
                    serde_json::json!({
...
pub fn router(state: &State) -> OpenApiRouter<State> {
    OpenApiRouter::new()
        ...
        .nest("/columns", columns::router(state))
